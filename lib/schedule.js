...
/**
 * Optional first-run date for create flow.
 * - blank → null (use next occurrence from now)
 * - +N → N calendar days from today (UTC) at hour:minute
...
export function parseStartDateInput(raw, { hour, minute, now = new Date() } = {}) {
  ...
  if (plus) {
    ...
    base.setUTCDate(base.getUTCDate() + Number(plus[1]));
    ...
    return "Use YYYY-MM-DD, +N days (e.g. +4), or leave blank.";
  }
