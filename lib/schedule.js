...
/**
 * True when next_run is so far in the past that sending would be a catch-up
 * dump after downtime (sleep/redeploy), not a timely delivery.
 */
export function isStaleDueEvent(nextRun, now = new Date(), graceMs = 120_000) {
  const dueAt = new Date(nextRun).getTime();
  if (Number.isNaN(dueAt)) return false;
  const at = now instanceof Date ? now.getTime() : new Date(now).getTime();
  return at - dueAt > graceMs;
}
