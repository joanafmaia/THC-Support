    ...
    new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        ...
        .addOptions(
          ...
          { label: "Weekly", value: "weekly", default: draft.repeat === "weekly" },
          { label: "Monthly", value: "monthly", default: draft.repeat === "monthly" }
        )
    ),
  ];
 ...
 * Time: HH:MM · Timezone: hours from UTC (0, -3, 1…)
 */
export function buildCreateSetupModal(draft = {}) {
  const time =
    draft.hour != null && draft.minute != null
      ...
      : "20:00";
  const tz = draft.timezoneOffset != null ? String(draft.timezoneOffset) : "0";
  ...
  const tzInput = new TextInputBuilder()
    ...
    .setLabel("UTC offset (Portugal 0, Brazil -3)")
    .setStyle(TextInputStyle.Short)
    .setRequired(true)
    .setMaxLength(3)
    .setPlaceholder("0")
